...
function buildListUrl(request: Request) {
  ...
  return `${INTERNAL_API_URL}/api/v1/email-access/allowlist?page=${page}&size=${size}&offset=${offset}`;
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const res = await fetch(`${INTERNAL_API_URL}/api/v1/email-access/allowlist`, {
      method: 'POST',
      headers: {
        'X-API-KEY': secretKey,
