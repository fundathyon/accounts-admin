...
export async function GET(request: Request) {
  ...
  try {
    const res = await fetch(`${INTERNAL_API_URL}/api/v1/roles?page=${page}&size=${size}&offset=${offset}`, {
      headers: {
        'X-API-KEY': secretKey,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const res = await fetch(`${INTERNAL_API_URL}/api/v1/roles`, {
      method: 'POST',
      headers: {
        'X-API-KEY': secretKey,
    ...
    return NextResponse.json(
      { success: false, error: { message: 'Error al crear el rol.' } },
      { status: 500 }
    );
  }
}
